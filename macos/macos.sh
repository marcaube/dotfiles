...
# sudo scutil --set ComputerName "MarcBook"
# sudo scutil --set HostName "MarcBook"
# sudo scutil --set LocalHostName "MarcBook"
# sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.smb.server NetBIOSName -string "MarcBook"
...
# Disabled: prevents the Mac from ever sleeping, so the screensaver runs forever
# instead of the system sleeping and turning the screen off. Manage sleep via
# System Settings → Battery instead.
...
# All disabled: vestigial Intel-era optimizations. On Apple Silicon, macOS
# manages hibernation and the sleep image itself; `pmset hibernatemode` is
# effectively ignored, and the sleep image path is SIP-protected so the rm
# silently fails. Risk vs. reward isn't worth it on a modern laptop.
...
defaults write NSGlobalDomain AppleLanguages -array "en_CA" "fr_CA"
